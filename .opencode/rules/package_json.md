---
title: "Package.json Formatting"
description: "Ensures that package.json keys are always sorted alphabetically with the wireit section placed at the end"
tags: [package-json, formatting, configuration, wireit]
version: "1.0.0"
...
This rule ensures that `package.json` keys are always sorted alphabetically, with the `wireit` section placed at the end.
...
2. Verify `wireit` section is at the end
3. Run `npm run validate` to confirm valid JSON

## Important Rules

- **Always keep `wireit` at the end** of the file
